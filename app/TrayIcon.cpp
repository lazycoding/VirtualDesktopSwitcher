...
TrayIcon::TrayIcon(HINSTANCE hInstance, const std::wstring& tooltip) : m_hInstance(hInstance), m_tooltip(tooltip) {
}
...
bool TrayIcon::initialize() {
    ...
    m_hwnd = CreateWindowW(className, nullptr, 0, 0, 0, 0, 0, nullptr, nullptr, m_hInstance, this);
    if (!m_hwnd) {
        return false;
    }
...
void TrayIcon::showNotification(const std::wstring& title, const std::wstring& message) {
    ...
    wcsncpy_s(m_notifyIconData.szInfo, sizeof(m_notifyIconData.szInfo) / sizeof(wchar_t), message.c_str(), _TRUNCATE);
    wcsncpy_s(
