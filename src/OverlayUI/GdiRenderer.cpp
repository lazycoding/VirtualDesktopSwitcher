...
COLORREF GdiRenderer::hexToCOLORREF(const std::wstring& hex) {
    if (hex.length() != 9 || hex[0] != L'#') {
        // Return default color if invalid format
        return RGB(100, 149, 237); // Cornflower Blue
    }

    // Parse RR, GG, BB components (ignore AA for COLORREF)
    wchar_t* end;
    ...
    return RGB(
        static_cast<BYTE>(rValue),
        static_cast<BYTE>(gValue),
        static_cast<BYTE>(bValue)
    );
}

void GdiRenderer::setTrailStyle(const std::wstring& colorHex, float lineWidth) {
    ...
    m_pen = CreatePen(PS_SOLID | PS_ENDCAP_ROUND | PS_JOIN_ROUND, 
                      static_cast<int>(m_lineWidth), m_trailColor);
}
