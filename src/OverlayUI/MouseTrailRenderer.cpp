...
bool MouseTrailRenderer::Initialize(HWND hwndParent) {
    ...
    SetWindowPos(m_hwnd, NULL, m_rcVirtual.left, m_rcVirtual.top, m_width,
        m_height, SWP_NOACTIVATE | SWP_NOZORDER);
...
void MouseTrailRenderer::CreateDIBAndRenderTarget() {
    ...
    m_hDIB = CreateDIBSection(hdcScreen, (BITMAPINFO*)&bi, DIB_RGB_COLORS,
        &m_pBits, NULL, 0);
...
MouseTrailRenderer::FPoint
MouseTrailRenderer::CatmullRom(const FPoint& p0, const FPoint& p1,
    const FPoint& p2, const FPoint& p3, float t) {
    float t2 = t * t;
    float t3 = t2 * t;
    return { 0.5f * ((2 * p1.x) + (-p0.x + p2.x) * t +
...
void MouseTrailRenderer::Render(const std::vector<POINT>& pts) {
    ...
    for (auto& p : pts)
        fpts.push_back(
            { (float)(p.x - m_rcVirtual.left), (float)(p.y - m_rcVirtual.top) });
        ...
        if (SUCCEEDED(geo->Open(&sink))) {
            ...
            for (size_t i = 1; i + 2 < fpts.size(); ++i) {
                for (int s = 1; s <= m_steps; ++s) {
                    ...
                    auto q =
                        CatmullRom(fpts[i - 1], fpts[i], fpts[i + 1], fpts[i + 2], t);
    ...
    UpdateLayeredWindow(m_hwnd, hdcScreen, &ptDst, &size, m_memDC, &ptSrc, 0, &bf,
        ULW_ALPHA);
    ReleaseDC(NULL, hdcScreen);
}
